...
const Chatbot = ({ onClose }) => {
  ...
  const handlePopupSubmit = async () => {
    ...
    try {
      ...
      setMessages(prev =>
        prev.map((msg, i) =>
          i === popupIdx ? { ...msg, feedbackGiven: true } : msg
        )
      );
      setShowPopup(false);
      setReason('');
  ...
  return (
    <div className="chatbot-container position-fixed bottom-1 end-1 m-3 shadow rounded-xl bg-white border"
      ...
      {/* Feedback Popup */}
      {showPopup && (
        <div className="position-absolute shadow rounded card-bg-color border border-1"
          style={{
            top: '30%',
            left: '50%',
          ...
          <div className="p-3" style={{ borderBottom: '1px solid var(--bs-border-color, #e0e0e0)', borderTopLeftRadius: '0.5rem', borderTopRightRadius: '0.5rem' }}>
            <h6 className="mb-2 card-text-Color" style={{ color: 'var(--bs-heading-color, #222)' }}>Why wasn't this helpful?</h6>
            <textarea className="form-control mb-3 card-bg-color border-1" rows="3"
              style={{ resize: 'vertical', color: 'var(--bs-body-color, #222)', background: 'inherit' }}
              value={reason}
