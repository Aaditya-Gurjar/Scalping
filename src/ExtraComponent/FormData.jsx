                ...
                {fields.map((field, index) => (
                  <React.Fragment key={index}>
                    {field?.headingtype == 1 ?
                            ...
                            <div className="iq-card-body row dark-card">
                              {field.data.map((item, index) => {
                                switch (item.type) {
                                  case "text":
                                    return (
                                      <div className={`col-lg-${item.col_size}`} key={index}>
                                    ...
                                    return (
                                      <div className={`col-lg-${item.col_size}`} key={index}>
                                        <div className="input-block row mb-3">
                                          <label className={`col-lg-${item.label_size}`} htmlFor={item.name}>
                                            {item.label}
                                            <span className="text-danger">*</span>
                                          </label>
                                          <div>
                                            ...
                                            >
                                              ...
                                              {item.options
                                                .filter((option) => item?.options1?.includes(option?.label))
                                                .map((option) => (
                                                  <option key={option.value} value={option.value} className="green">
                                                    {option.label}
                                                  </option>
                                                ))}
                                              {item.options
                                                .filter((option) => !item?.options1?.includes(option?.label))
                                                .map((option) => (
                                                  <option key={option.value} value={option.value} className="black">
                                                    {option.label}
                                                  </option>
                                                ))}
                                            </select>
                      ...
                      : field?.headingtype == 2 ?
                        <div className="iq-card" style={{ boxShadow: "rgba(0, 0, 0, 0.02) 0px 1px 3px 0px, rgba(27, 31, 35, 0.15) 0px 0px 0px 1px" }}>
                                          ...
                                          <div className="input-block row mb-3">
                                            ...
                                            <div>
                                              ...
                                              >
                                                ...
                                                {item.options
                                                  .filter((option) =>
                                                    item?.options1?.includes(option?.label)
                                                  )
                                                  .map((option) => (
                                                    <option
                                                      className="green"
                                          ...
                                          <div className="row">
                                                ...
                                                <div className="mb-4">
                                                  <div className="form-check custom-checkbox">
                                                    <input
                                                      ...
                                                      id="defaultCheckbox"
                                                      {...formik.getFieldProps(field.name)}
                                                      checked={formik.values[field.name]?.length === field.options.length} // Check if all options are selected
                                                      onChange={() => {
                                                        if (formik.values[field.name]?.length === field.options.length) {
                                                          formik.setFieldValue(field.name, []); // Uncheck all
                                                        } else {
                                                          formik.setFieldValue(field.name, field.options.map(option => option.value)); // Check all
                                                        }
                                                      }}
                                                    />
                                            ...
                                            </div>
                                            {
                                              field.options.map((opData) => (
                                                <div className="col-lg-3 col-md-4 col-sm-6" key={opData.value}>
                                                  <div className="row d-flex justify-content-start">
                                                    <div className="mb-4">
                                                      <div className="form-check custom-checkbox">
                                                        <input
                                                          ...
                                                          id={opData.label}
                                                          {...formik.getFieldProps(field.name)}
                                                          checked={formik.values[field.name]?.includes(opData.value)}
                                                          onChange={() => {
                                                            const selectedBrokers = [...formik.values[field.name]];
                                                            if (selectedBrokers.includes(opData.value)) {
                                                              formik.setFieldValue(field.name, selectedBrokers.filter(value => value !== opData.value));
                                                            } else {
                                                              formik.setFieldValue(field.name, [...selectedBrokers, opData.value]);
                                                            }
                                                          }}
                                                        />
                                                        <label className="form-check-label" htmlFor={opData.label}>
                                                          {opData.label}
                                                        </label>
                                                      </div>
                                                      {formik.errors[field.name] && (
